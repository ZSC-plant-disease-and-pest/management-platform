...
export default defineComponent({
  ...
  setup () {
    ...
    const size = ref(10);
    // 标签集参数
    const labelCollectionParams = reactive({
      page: 0,
      size: 10
    } as labelCollectionParams);
    // 请求标签集
    const getLabelCollection = () => {
      loading.value = true;
      labelCollectionHttp.searchLabelCollection(labelCollectionParams)
        .then((response: any) => {
    ...
    const sortChange = (params: any) => {
      if (params.prop === null) {
        // 无规则
        labelCollectionParams.sort = '';
      } else {
        // 排序规则
        labelCollectionParams.sort = params.prop + ',' + (params.order === 'descending' ? 'desc' : 'asc');
      }
      getLabelCollection();
    };
    // 新增
    ...
    const remove = (selectedIds: any) => {
      if (selectedIds.length === 0) {
        ...
        loading.value = true;
        labelCollectionHttp.deleteLabelCollection(selectedIds.join(','))
          .then(() => {
    ...
    const search = (data: any) => {
      for (const index in data) {
        if (data[index].name === 'name') {
          labelCollectionParams.name = data[index].value === '' ? undefined : data[index].value;
        }
      }
      getLabelCollection();
    };
    ...
    const handleSizeChange = (newSize: any) => {
      labelCollectionParams.size = newSize;
      labelCollectionParams.page = 0;
      size.value = newSize;
      page.value = 1;
      getLabelCollection();
    };
    ...
    const handleCurrentChange = (newPage: any) => {
      labelCollectionParams.page = newPage;
