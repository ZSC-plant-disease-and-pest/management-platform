  ...
  >
    <el-card class="base-card">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="病害数据集名称：" prop="name">
            <el-input
              class="input-common"
              v-model="form.name"
              placeholder="请输入病害数据集名称"
              v-if="type === 'add'"
            />
            <el-input
              class="input-common"
...
export default defineComponent({
  ...
  setup () {
    ...
    const state = reactive({
      ...
      rules: {
        name: [
          { required: true, message: '请输入病害数据集名称', trigger: ['blur', 'change'] }
        ]
      },
      ...
      state.formRef.validate().then((valid: boolean) => {
        if (valid) {
          ...
          if (route.path.split('/').slice(-1)[0] === 'add') {
            datasetHttp.createDiseaseDataset(state.form)
              .then(() => {
            ...
            datasetHttp.updateDiseaseDataset(state.form)
              .then(() => {
                state.status = 'complete';
              })
              .finally(() => {
