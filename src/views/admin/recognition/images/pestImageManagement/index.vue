...
export default defineComponent({
  ...
  setup () {
    ...
    const add = () => {
      state.dialogRef.openDialog('pest');
    };
    const remove = (selectedIds: any) => {
      if (selectedIds.length === 0) {
        ...
        state.isLoading = true;
        datasetHttp.deletePest(selectedIds.join(','))
          .then(() => {
    ...
    const check = (data: any) => {
      window.open(`http://localhost:8082/pest/detail/${data.informationId}`, '_blank');
    };
    const checkDateset = (data: any) => {
      router.push({
        path: `/admin/recognition/datasetDetail/pest/${data.id}`,
        query: {
          name: data.name,
